...
- [src/similarity/](src/similarity/) — **the heart** (same logic as Python, rayon replaces multiprocessing).
  ...
  - `clustering.rs` — `cluster_findings`/`filter_clusters` (union-find over `function.identity`; only runs when `cluster_findings` is on) and `build_groups` (derives stable, re-numbered `CloneGroup`s from findings for the reporters/stats — presentation only, **no** detection-baseline impact; unclustered → one singleton group per finding, clustered → findings sharing a function merge; sorts by the full location-identity vector then assigns sequential ids).
- [src/reporting/](src/reporting/) — `schema.rs` (`SCHEMA_VERSION = env!("CARGO_PKG_VERSION")`). `compare.rs` (`select_compare` → `best_match` for rendering). `json.rs` (`write_json`: `{schema_version, groups, stats, config, timing, degradations}`; findings nest under `groups[].findings[]` via `build_groups`, each with a `similar`-crate unified diff — the flat top-level `findings` array was replaced). `sarif.rs` (`write_sarif`: SARIF 2.1.0, `note`-level results; still one result per finding, unaffected by grouping). `html.rs` (`write_html`: self-contained inline CSS/JS, `DiffOp` side-by-side diff, client-side sort; self-clone aware via `SelfCloneOccurrences`; renders clone-group chrome under `--cluster`, flat per-finding cards otherwise).
