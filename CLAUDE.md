...
- **Self-match filtering / exact-duplicate suppression:** retrieval skips a snippet whose `snippet_hash` equals a neighbor's — and because `snippet_hash = hash_text(text)`, this also suppresses matches between *different* functions with byte-identical normalized text (an exact duplicate is not reported). Rollup then keeps *same-function* self-clones only when line ranges are disjoint and drops *same-file cross-function* range overlaps as containment.
