...
class BruteIndex(VectorIndex):
    def __init__(self) -> None:
        self._ids: list[str] = []
        self._matrix: NDArray[np.float64] | None = None  # (N, D), raw vectors
        self._norms: NDArray[np.float64] | None = None  # (N,), precomputed L2 norms
    ...
    def build(self, vectors: list[Embedding], ids: list[str]) -> None:
        ...
        if not vectors:
            ...
            self._norms = None
            return
        self._matrix = np.asarray([v.vector for v in vectors], dtype=np.float64)
        norms = cast(NDArray[np.float64], np.linalg.norm(self._matrix, axis=1))
        self._norms = cast(NDArray[np.float64], np.where(norms == 0, 1.0, norms))
    ...
    def query(self, vector: Embedding, k: int) -> list[tuple[str, float]]:
        if self._matrix is None or self._norms is None or len(self._ids) == 0:
            return []
        q = np.asarray(vector.vector, dtype=np.float64)
